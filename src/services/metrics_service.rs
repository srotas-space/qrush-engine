...
pub async fn render_metrics(query: web::Query<MetricsQuery>) -> impl Responder {
    ...
    let active_queues: Vec<String> = conn.smembers(QUEUES_SET).await.unwrap_or_default();
    
    // Get configured queues from Redis config keys
    let config_pattern = format!("{QUEUE_CONFIG_PREFIX}:*");
    let config_keys: Vec<String> = conn.keys(&config_pattern).await.unwrap_or_default();
    let configured_queues: Vec<String> = config_keys
        .into_iter()
        .filter_map(|key| key.strip_prefix(&format!("{QUEUE_CONFIG_PREFIX}:")).map(String::from))
        .collect();
    ...
    for queue in &paginated_queues {
        let success_key = format!("{SUCCESS_LIST_PREFIX}:{}", queue);
        let failed_key = format!("{FAILED_LIST_PREFIX}:{}", queue);
        let retry_key = format!("{RETRY_LIST_PREFIX}:{}", queue);
        let pending_key = format!("{PREFIX_QUEUE}:{}", queue);
...
pub async fn render_metrics_for_queue(
    ...
    let key = format!("{PREFIX_QUEUE}:{}", queue);
...
pub async fn render_scheduled_jobs(query: web::Query<PaginationQuery>) -> impl Responder {
    ...
    let job_ids: Vec<String> = conn
        .zrangebyscore(DELAYED_JOBS_KEY, 0, now)
        .await
        .unwrap_or_default();
    ...
    for jid in job_ids {
        if let Ok(data) = conn.get::<_, String>(format!("{PREFIX_JOB}:{}", jid)).await {
...
pub async fn render_dead_jobs(query: web::Query<PaginationQuery>) -> impl Responder {
    ...
    let all_jobs: Vec<String> = match conn.lrange(FAILED_JOBS_LIST, 0, -1).await {
        Ok(jobs) => jobs,
        Err(e) => {
...
pub async fn render_worker_status() -> impl Responder {
    ...
    let worker_pattern = format!("{WORKER_PREFIX}:*");
    let keys: Vec<String> = conn.keys(&worker_pattern).await.unwrap_or_default();
    let mut workers = Vec::new();

    for key in keys {
        if let Ok(status_json) = conn.get::<_, String>(&key).await {
            if let Ok(mut status) = serde_json::from_str::<WorkerStatus>(&status_json) {
                status.id = key.replace(&format!("{WORKER_PREFIX}:"), "");
                workers.push(status);
            }
        }
    }
    ...
    match action {
        "delete" => {
            if conn.exists(&job_key).await.unwrap_or(false) {
                ...
                let logs_key = format!("{LOGS_PREFIX}:default");
                let _: () = conn.lpush(&logs_key, format!("[{}] 🗑️ Job {} deleted", Utc::now(), job_id)).await.unwrap_or_default();
                let _: () = conn.ltrim(&logs_key, 0, 99).await.unwrap_or_default();
...
pub async fn get_metrics_summary() -> impl Responder {
    ...
    let total_jobs: usize = conn.get(COUNTER_TOTAL_JOBS).await.unwrap_or(0);
    let success_jobs: usize = conn.get(COUNTER_SUCCESS).await.unwrap_or(0);
    let failed_jobs: usize = conn.get(COUNTER_FAILED).await.unwrap_or(0);

    let queues: Vec<String> = conn.smembers(QUEUES_SET).await.unwrap_or_default();
    let mut scheduled_jobs = 0;
    for queue in &queues {
        let len: usize = conn.llen(format!("{PREFIX_QUEUE}:{}", queue)).await.unwrap_or(0);
        scheduled_jobs += len;
    }

    let worker_pattern = format!("{WORKER_PREFIX}:*");
    let worker_keys: Vec<String> = conn.keys(&worker_pattern).await.unwrap_or_default();
    ...
    for i in (0..7).rev() {
        ...
        let total_key = format!("{STATS_JOBS_PREFIX}:{}", date_str);
        let failed_key = format!("{STATS_JOBS_FAILED_PREFIX}:{}:failed", date_str);
...
pub async fn render_failed_jobs(query: web::Query<PaginationQuery>) -> impl Responder {
    ...
    for queue in &queues {
        let key = format!("{FAILED_LIST_PREFIX}:{}", queue);
...
pub async fn render_retry_jobs(query: web::Query<PaginationQuery>) -> impl Responder {
    ...
    let mut all_jobs: Vec<String> = Vec::new();
    for queue in &queues {
        let key = format!("{RETRY_LIST_PREFIX}:{}", queue);
        let ids: Vec<String> = conn.lrange(&key, 0, -1).await.unwrap_or_default();
        all_jobs.extend(ids);
    }
